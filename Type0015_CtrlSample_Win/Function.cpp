...
BOOL Search_Module( void* Path )
{
	...
	CFStringRef		fileName = CFSTR("Type0015 Module.bundle");
...
BOOL SetWBPresetDataCapability( LPRefObj pRefSrc, ULONG ulCapID )
{
	...
	// Check operations
	if( ulCapID == kNkMAIDCapability_WBPresetData )
	{
	...
	{
		pCapInfo = GetCapInfo( pRefSrc, kNkMAIDCapability_MovieWBPresetData );
		if ( pCapInfo == NULL ) return false;
	}
	...
	{
		if ( !CheckCapabilityOperation( pRefSrc, kNkMAIDCapability_MovieWBPresetData, kNkMAIDCapOperation_Set ) ) return false;
	}
	...
	{
		bRet = Command_CapSet( pRefSrc->pObject, kNkMAIDCapability_MovieWBPresetData, kNkMAIDDataType_GenericPtr, (NKPARAM)&(stPresetData), NULL, NULL );
	}
...
BOOL SetPictureControlDataCapability( LPRefObj pRefObj, NkMAIDPicCtrlData* pPicCtrlData, char* filename, ULONG ulCapID )
{
	...
	else if( ulCapID == kNkMAIDCapability_PictureControlDataEx )
	{
	...
	else if( ulCapID == kNkMAIDCapability_MoviePictureControlData )
	{
		LPNkMAIDCapInfo	pCapInfo = GetCapInfo( pRefObj, kNkMAIDCapability_MoviePictureControlData );
		if ( pCapInfo == NULL ) return false;

		// check data type of the capability
		if ( pCapInfo->ulType != kNkMAIDCapType_Generic ) return false;
		// check if this capability supports CapSet operation.
		if ( !CheckCapabilityOperation( pRefObj, kNkMAIDCapability_MoviePictureControlData, kNkMAIDCapOperation_Set ) ) return false;
	}
	else
	{
		LPNkMAIDCapInfo	pCapInfo = GetCapInfo( pRefObj, kNkMAIDCapability_MoviePictureControlDataEx );
		if ( pCapInfo == NULL ) return false;

		// check data type of the capability
		if ( pCapInfo->ulType != kNkMAIDCapType_Generic ) return false;
		// check if this capability supports CapSet operation.
		if ( !CheckCapabilityOperation( pRefObj, kNkMAIDCapability_MoviePictureControlDataEx, kNkMAIDCapOperation_Set ) ) return false;
	}
	...
	if( ulCapID == kNkMAIDCapability_PictureControlData || ulCapID == kNkMAIDCapability_MoviePictureControlData )
	{
	...
	{
		// Set Picture Control data.
		bRet = Command_CapSet( pRefObj->pObject, kNkMAIDCapability_MoviePictureControlData, kNkMAIDDataType_GenericPtr, (NKPARAM)pPicCtrlData, NULL, NULL );
		if( bRet == false ) 
		{
			printf( "\nFailed in setting Movie Picture Contol Data.\n" );
		}
	}
	else  if( ulCapID == kNkMAIDCapability_MoviePictureControlDataEx )
	{
		// Set Picture Control data.
		bRet = Command_CapSet( pRefObj->pObject, kNkMAIDCapability_MoviePictureControlDataEx, kNkMAIDDataType_GenericPtr, (NKPARAM)pPicCtrlData, NULL, NULL );
		if( bRet == false ) 
		{
			printf( "\nFailed in setting Movie Picture Contol DataEx.\n" );
		}
	}
...
BOOL GetPictureControlDataCapability( LPRefObj pRefObj, NkMAIDPicCtrlData* pPicCtrlData, ULONG ulCapID )
{
	...
	else if( ulCapID == kNkMAIDCapability_MoviePictureControlData  )
	{
		...
		if ( pCapInfo->ulType != kNkMAIDCapType_Generic ) return false;
		// check if this capability supports CapGet operation.
		if ( !CheckCapabilityOperation( pRefObj, kNkMAIDCapability_MoviePictureControlData, kNkMAIDCapOperation_Get ) ) return false;
	}
	else
	{
		LPNkMAIDCapInfo	pCapInfo = GetCapInfo( pRefObj, kNkMAIDCapability_MoviePictureControlDataEx );
		...
		if ( pCapInfo->ulType != kNkMAIDCapType_Generic ) return false;
		// check if this capability supports CapGet operation.
		if ( !CheckCapabilityOperation( pRefObj, kNkMAIDCapability_MoviePictureControlDataEx, kNkMAIDCapOperation_Get ) ) return false;
	}
	...
	{
		// Get Picture Control data.
		bRet = Command_CapGet( pRefObj->pObject, kNkMAIDCapability_MoviePictureControlData, kNkMAIDDataType_GenericPtr, (NKPARAM)pPicCtrlData, NULL, NULL );
		if( bRet == false ) 
		{
			printf( "\nFailed in getting Movie Picture Control Data.\n" );
			free( pPicCtrlData->pData );
			return false;
		}
	}
	...
	{
		// Get Picture Control dataEx.
		bRet = Command_CapGet( pRefObj->pObject, kNkMAIDCapability_MoviePictureControlDataEx, kNkMAIDDataType_GenericPtr, (NKPARAM)pPicCtrlData, NULL, NULL );
		if( bRet == false ) 
		{
			printf( "\nFailed in getting Movie Picture Control DataEx.\n" );
			free( pPicCtrlData->pData );
			return false;
		}
	}
...
BOOL GetPictureControlInfoCapability( LPRefObj pRefSrc, ULONG ulCapID )
{
	char buf[256], filename[256];
	...
	if( ulCapID == kNkMAIDCapability_GetPicCtrlInfo)
	{
	...
	{
		pCapInfo = GetCapInfo( pRefSrc, kNkMAIDCapability_GetMoviePicCtrlInfo );
		if ( pCapInfo == NULL ) return false;
	}
	...
	{
		if ( !CheckCapabilityOperation( pRefSrc, kNkMAIDCapability_GetMoviePicCtrlInfo, kNkMAIDCapOperation_Get ) ) return false;
	}
	...
	{
		bRet = Command_CapGet( pRefSrc->pObject, kNkMAIDCapability_GetMoviePicCtrlInfo, kNkMAIDDataType_GenericPtr, (NKPARAM)&(stPicCtrlInfo), NULL, NULL );
		if( bRet == false ) 
		{
			free( stPicCtrlInfo.pData );
			return false;
...
BOOL DeleteCustomPictureControlCapability( LPRefObj pRefSrc, ULONG ulCapID )
{
	...
	if( ulCapID == kNkMAIDCapability_DeleteCustomPictureControl )
	{
	...
	{
		bRet = Command_CapSet( pRefSrc->pObject, kNkMAIDCapability_DeleteMovieCustomPictureControl, kNkMAIDDataType_Unsigned, (NKPARAM)ulValue, NULL, NULL );
	}
	return bRet;
}
//------------------------------------------------------------------------------------------------------------------------------------
